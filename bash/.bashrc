...
# FZF configuration
export FZF_DEFAULT_COMMAND='fd --exclude={.git,.cache,.xmake,.zig-cache,build,tmp,node_modules} --type f -H'
...
add_to_path "$HOME/.local/applications/aseprite"
...
update_prompt() {
    local user_color="$BRIGHT_GREEN"
    local host_color="$BRIGHT_BLUE"
    local path_color="$BRIGHT_CYAN"
    local prompt_symbol="❯"
...
# System information
sysinfo() {
    echo -e "${C_BRIGHT_CYAN}System Information:${C_RESET}"
    echo -e "Hostname: $(hostname)"
    echo -e "Uptime: $(uptime -p)"
    echo -e "Kernel: $(uname -sr)"
    echo -e "Shell: $SHELL"
    echo -e "CPU: $(grep 'model name' /proc/cpuinfo | head -1 | cut -d: -f2 | sed 's/^ *//')"
    echo -e "Memory: $(free -h | awk 'NR==2{printf "%.1f/%.1f GB (%.2f%%)", $3/1024/1024, $2/1024/1024, $3*100/$2}')"
...
# Welcome message
echo -e "${C_BRIGHT_GREEN}Welcome back, ${USER}!${C_RESET}"
